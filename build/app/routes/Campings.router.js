...
CampingsRouter.get('/categorias', async (_req, res) => {
    try {
        res.status(200).json(await (0, Campings_service_1.getCampingsCategorias)());
    }
    catch (_a) {
        res.status(404).json({ error: `no se pudo en http://localhost/api/camping/categorias` });
    }
});
CampingsRouter.get('/tarifas', async (_req, res) => {
    try {
        res.status(200).json(await (0, Campings_service_1.getCampingTarifas)());
    }
    catch (_a) {
        res.status(404).json({ error: `no se pudo en http://localhost/api/camping/tarifas` });
    }
});
CampingsRouter.get('/abierto', async (_req, res) => {
    try {
        res.status(200).json(await (0, Campings_service_1.getCampingAbiertoPeriodo)());
    }
    catch (_a) {
        res.status(404).json({ error: `no se pudo en http://localhost/api/camping/abierto` });
    }
});
CampingsRouter.get('/agua_caliente', async (_req, res) => {
    try {
        res.status(200).json(await (0, Campings_service_1.getCampingPeriodoAguaCaliente)());
    }
    catch (_a) {
