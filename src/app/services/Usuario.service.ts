...
export const getUser = async (id: string): Promise<datosUsuario> => {
  const [user]: [user: datosUsuario[]] = 
    await sequelize.query(
      `SELECT U.id, U.email, U.nombre_completo, U.numero_celular, U.habilitado, U.dni, T.id AS TipoUsuarioId  FROM Usuarios AS U INNER JOIN Tipo_usuarios AS T ON T.id=U.TipoUsuarioId WHERE U.id=${id};`
    );
