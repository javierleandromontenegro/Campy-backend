...
import { getProvincias, getProvinciasCantCampings } from '../services/Provincias.service';
...
ProvinciaRouter.get('/campings', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(404).json({error: 'no se pudo en http://localhost/api/provincias/campings'});
  }
});
