...
LocalidadesRouter.get(
  ...
  async (req: Request<{ idProvincia: string }>, res: Response) => {
    ...
    try {
      ...
      res.status(404).json({
        error:
          "No se pudo en http://localhost:3001/api/localidades/ConCampings/idprovincia",
      });
    }
  }
);
...
LocalidadesRouter.get(
  ...
  async (req: Request<{ idProvincia: string }>, res: Response) => {
    ...
    try {
      ...
      res.status(404).json({
        error: "No se pudo en http://localhost/api/localidades/idprovincia",
      });
    }
  }
);
...
LocalidadesRouter.post(
  ...
  checkoutUser,
  checkoutAdmin,
  async (req: Request<{}, createLocalidad>, res: Response) => {
    try {
